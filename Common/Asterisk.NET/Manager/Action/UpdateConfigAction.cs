...
namespace Asterisk.NET.Manager.Action
{
	...
	public class UpdateConfigAction : ManagerActionResponse
	{
		...
		public void AddCommand(string action, string category, string variable, string value, string match)
		{
            ...
            string number = string.Format("{0:D6}",i);
			if (!string.IsNullOrEmpty(action))
                actions.Add("Action-" + number, action);

			if (!string.IsNullOrEmpty(category))
                actions.Add("Cat-" + number, category);

			if (!string.IsNullOrEmpty(variable))
                actions.Add("Var-" + number, variable);

			if (!string.IsNullOrEmpty(value))
                actions.Add("Value-" + number, value);

			if (!string.IsNullOrEmpty(match))
                actions.Add("Match-" + number, match);
		}
