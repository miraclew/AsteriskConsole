...
namespace VisualAsterisk.Core.Deploy
{
    public class FirstTimeConfigInstaller : IConfigInstaller
    {
        ...
        public void Install()
        {
            sshCommandExecutor.Execute("cp /etc/asterisk/http.conf /etc/asterisk/http.conf.bak");
            sshCommandExecutor.Execute("cp /etc/asterisk/manager.conf /etc/asterisk/manager.conf.bak");
